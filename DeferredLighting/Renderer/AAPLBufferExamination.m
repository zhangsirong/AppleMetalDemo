...
- (void) loadMetalState
{
    ...
    // Set up pipeline to render lightVolumes
    {
        ...
        NSAssert(_lightVolumeVisualizationPipelineState,
                 @"Failed to create light volume visualization render pipeline state, error %@",
                 error);
    }
    ...
    // Set up pipelines to display raw GBuffers
    {
        ...
        NSAssert(_textureRGBPipelineState,
                 @"Failed to create texture RGB render pipeline state, error %@",
                 error);
        ...
        NSAssert(_textureAlphaPipelineState,
                 @"Failed to create texture alpha render pipeline state, error %@", error);
        ...
        NSAssert(_textureDepthPipelineState,
                 @"Failed to create depth texture render pipeline state, error %@", error);
     }
...
- (void)renderFullLightVolumeExaminationWithCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
{
    ...
    renderEncoder.label = @"All light volumes";
...
{
     NSAssert(_offscreenDrawable, @"Need Offscreen Drawable");
    ...
    id<MTLRenderCommandEncoder> renderEncoder =
        [commandBuffer renderCommandEncoderWithDescriptor:renderPassDescriptor];
    renderEncoder.label = @"Examination Buffer Presentation";
    ...
    switch(_mode)
    {
        ...
        case AAPLExaminationModeDisabled:
            NSAssert(0, @"This method should not be called when Examination Mode is Disabled");
            break;
    }
